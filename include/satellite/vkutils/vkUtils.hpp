...
namespace NSM {
    ...
    void destroyBuffer(BufferType& buffer) {
        if (buffer && buffer->initialized) {
            vmaDestroyBuffer(buffer->device->allocator, buffer->buffer, buffer->allocation); buffer->initialized = false;
        }
    }

    void destroyTexture(TextureType& image) {
        if (image && image->initialized) {
            vmaDestroyImage(image->device->allocator, image->image, image->allocation); image->initialized = false;
        }
    }
