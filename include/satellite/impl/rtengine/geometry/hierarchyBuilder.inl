...
void HieararchyBuilder::init(DeviceQueueType &device)
{
    ...
    pipelineLayout = device->logical.createPipelineLayout(vk::PipelineLayoutCreateInfo().setPSetLayouts(descriptorSetLayouts.data()).setSetLayoutCount(descriptorSetLayouts.size()));
...
void HieararchyBuilder::build(glm::dmat4 optproj)
{
...

    {
        ...
        memoryCopyCmd(command, bvhBoxWorking, bvhBoxStorage, {0, 0, strided<bbox>(nodeCount)});
        flushCommandBuffer(device, command, true);
    }
}
