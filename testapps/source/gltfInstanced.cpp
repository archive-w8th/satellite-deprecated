...
namespace SatelliteExample {
    ...
    int GltfViewer::parseArguments(const int32_t& argc, const char ** argv, GLFWwindow * window) {
        args::ArgumentParser parser("This is a test rendering program.", "BE ACCURE WITH HUGE MODELS!");
        ...
        if (help || argc <= 1) { std::cout << parser; glfwDestroyWindow(window); glfwTerminate(); system("pause"); exit(0); } // required help or no arguments
        if (model_input == "") std::cerr << "No model found :(" << std::endl;

        //model_input = "DamagedHelmet-processed.gltf";
        //mscale = 1.f;
        return 1;
    }
    ...
    void GltfViewer::process() {
        ...
        glm::vec4 translationMod = glm::vec4(2.f, 2.f, 2.f, 0.f);
        float rotationOffsetRad = 1.5f;
        ...
        // push helments instances with rotations
        for (int x = -3; x <= 1;x += 2) {
            for (int y = -3; y <= 1; y += 2) {
                for (int z = -3; z <= 1; z += 2) {

                    glm::mat4 matrix(1.0);
                    matrix *= glm::translate(translationMod.xyz() * glm::vec3(x, y, z));
                    matrix *= glm::rotate(float(t / 1000.f)*rotationSpeed, rotationOffsetRad + glm::vec3(x, y, z));
                    matrix *= glm::scale(glm::vec3(mscale));
