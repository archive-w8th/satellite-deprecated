...
namespace NSM {
    namespace rt {
        ...
        BufferComposer<BINDING,STRUCTURE>::BufferComposer(DeviceQueueType& device) {
            this->device = device;
            cache = createBuffer(device, strided<STRUCTURE>(256), vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eTransferDst | vk::BufferUsageFlagBits::eTransferSrc, VMA_MEMORY_USAGE_GPU_ONLY);
            stager = createBuffer(device, strided<STRUCTURE>(256), vk::BufferUsageFlagBits::eTransferDst | vk::BufferUsageFlagBits::eTransferSrc, VMA_MEMORY_USAGE_CPU_TO_GPU);
        }
        ...
        BufferSpace::BufferSpace(DeviceQueueType& device, const size_t spaceSize) {
            ...
            regionsBuffer = createBuffer(device, strided<BufferRegion>(256), vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eTransferDst | vk::BufferUsageFlagBits::eTransferSrc, VMA_MEMORY_USAGE_GPU_ONLY);
            regionsStage = createBuffer(device, strided<BufferRegion>(256), vk::BufferUsageFlagBits::eTransferDst | vk::BufferUsageFlagBits::eTransferSrc, VMA_MEMORY_USAGE_CPU_TO_GPU);
