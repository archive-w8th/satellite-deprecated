...
void TriangleHierarchy::buildBVH(glm::dmat4 optproj)
{
    ...
    for (int j = 0; j < 8; j++)
    {
        buildSubmitInfos.push_back(vk::SubmitInfo()
                                       ...
                                       .setPCommandBuffers(&buildCommand)); // build level command
        buildSubmitInfos.push_back(vk::SubmitInfo()
                                       .setWaitSemaphoreCount(0)
                                       .setCommandBufferCount(1)
                                       .setPCommandBuffers(&copyCounterCommand)); // copying commands
    }
    buildSubmitInfos.push_back(vk::SubmitInfo()
                                   .setWaitSemaphoreCount(0)
                                   .setCommandBufferCount(1)
                                   .setPCommandBuffers(&stagingCounterCommand)); // getting to stage buffer command
