...
// blocks managment
layout ( std430, binding = 1, set = 0 )  buffer BlocksSSBO { RBlock rayBlocks[]; }; // block headers and indices
...
layout ( std430, binding = 11, set = 0 )  buffer UnorderedSSBO { int unorderedRays[]; };
...
bool checkIllumination(in int block, in int bidx){
    ...
    return (nd >= 0 && block >= 0 ? mlength(rayBlockNodes[block][nd].data.final.xyz) >= 0.0001f : false);
}
...
int createBlock(in int blockBinId){
    ...
    if (LANE_IDX == activeLane) atomicMax(arcounter.mT, 0);
